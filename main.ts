...
// 1. Defining a type alias named Product representing a product with the following properties.
type Product = {
    name: string,
    price: number,
    inventory: {
...
const productA = [
    {
        ...
        inventory: {
            stock: 10,
            colorOptions: ["Black","White","Red","Blue"],
            changeColor: function (color1:string,color2:string){
...
// 1 . Defining a type named Student with the following properties
type Student = {
    name: string,
    grades: number[]
}
...
// 1. Defining a type alias named Employee with following properties.
type Employee = {
    name: string,
    hoursWorked: number,
    hourlyRate: number,
...
function calculateSalary(employee:any){
...
function UpdatedHoursAndSalary(employees: Employee[],employeeName:string , updatedHours:number) : void {
    employees.forEach(employee => {
